...
# ── Agent Container Runtime - Cloud Install ────────────────────────
...
# Usage:
#   curl -fsSL <raw-url>/cloud-install.sh | bash
#   curl -fsSL <raw-url>/cloud-install.sh | bash -s -- --api-key sk-xxx --model GLM-5.1

# Keep in sync with agent-runtime/VERSION
VERSION="0.2.5"
IMAGE_REPO="ghcr.io/yiyan-yixing/agent-runtime"
IMAGE_TAG="latest"
...
show_banner() {
    ...
    echo -e "${CYAN} / _ \\| |_| |_ ___ _ __   __| | __ _ _ __| |_ ${NC}"
    echo -e "${CYAN}| |_| |  _|  _/ _ \\ '_ \\ / _\` |/ _\` | '__| __|${NC}"
    echo -e "${CYAN}|  _  \\ |_| ||  __/ | | | (_| | (_| | |  | |_ ${NC}"
    echo -e "${CYAN}|_| |_|\\__|\\__\\___|_| |_|\\__,_|\\__,_|_|   \\__|${NC}"
    echo ""
    echo "  Agent Container Runtime v${VERSION} - Cloud Install"
    echo ""
}
...
show_help() {
    cat <<EOF
Usage: cloud-install.sh [options]
...
Examples:
  ...
  bash cloud-install.sh --api-key sk-xxx --model GLM-5.1
  bash cloud-install.sh --registry-user foo --registry-pass bar --tag 0.2
EOF
    exit 0
}
