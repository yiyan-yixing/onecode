(function () {
  ...
  var INTERACTIVE_WINDOW_MS = 100;       // Flush within 100ms of user input
  var _lastInputTime = 0;                // Timestamp of last user keystroke
  ...
  function connectWs() {
    ...
    ws.onmessage = function (event) {
      ...
      if (bytes[0] === FRAME_PTY) {
        ...
        // Interactive fast path: response to recent user input (backspace, arrows, etc.)
        // Flush immediately to avoid RAF delay that makes typing feel sluggish
        if (Date.now() - _lastInputTime < INTERACTIVE_WINDOW_MS) {
          if (_writeQueue.length > 0) {
            flushWriteQueue();
          }
          term.write(payload);
