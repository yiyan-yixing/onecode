...
const FLUSH_INTERACTIVE_MS = 100;         // Flush within 100ms of user input (interactive response)
...
class PtyManager extends EventEmitter {
  constructor() {
    ...
    this._lastInputTime = 0;           // Timestamp of last user input (write())
  }
  ...
  _createPty() {
    ...
    p.onData((data) => {
      ...
      if (self._pendingBytes >= FLUSH_IMMEDIATE_BYTES) {
        ...
        self._doFlush(p);
      } else if (Date.now() - self._lastInputTime < FLUSH_INTERACTIVE_MS) {
        // Interactive response (e.g. backspace, arrow keys) — flush immediately
        // to avoid the 50ms timer making typing feel sluggish
        clearTimeout(self._flushTimer);
        self._flushTimer = null;
        self._doFlush(p);
